...
$where = "WHERE 1=1";
 if($criterio!="")
     $where.= " AND razon_social LIKE '$criterio%'";
	 
include"../clases/ConexionDatos.php";
$objConexion=Conectarse();

$query_pag_data = "SELECT * from gc_proveedores " . $where . " order by razon_social LIMIT $start, $per_page";
$result_pag_data = mysqli_query($objConexion,$query_pag_data) or die('MySql Error' . mysql_error());
$msg = "";
$msg .= '<div class="jtable-title"><div class="jtable-title-text">Proveedores<input type="text" class="criterio" /><input type="button" id="filtrar" class="go_filtro" value="Filtrar"/><img style="float:right;padding:8px;cursor:copy" src="../Imagenes/add.png" ';
$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/frmActualizarrproveedores.php&x=1';
$msg .= '\'"></div></div>';
...
while ($row = mysqli_fetch_array($result_pag_data)) {
	...
	$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/frmActualizarrproveedores.php&amp;Id=';
	$msg .= $row['nit'];
	$msg .= '\'">';
	...
	$msg .= ' onclick="location.href=\'../controlador/coproveedores.php?boton=eliminar&amp;Id=';
	$msg .= $row['nit'];
	$msg .= '\'">';
...
/* --------------------------------------------- */
$query_pag_num = "SELECT COUNT(*) AS count FROM gc_proveedores " . $where;
