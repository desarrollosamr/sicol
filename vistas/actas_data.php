...
$query_pag_data = "SELECT gc_produccion.id as codigo, gc_produccion.fecha as fecha, gc_produccion.acta as acta, gc_productos.nombre as producto, gc_produccion.cantidad as cantidad from gc_produccion left join gc_productos on gc_produccion.producto = gc_productos.codigo " . $where . " order by fecha desc LIMIT $start, $per_page";
$result_pag_data = mysqli_query($objConexion,$query_pag_data) or die('MySql Error' . mysql_error());
$msg = "";
$msg .= '<div class="jtable-title"><div class="jtable-title-text">Produccion<input type="text" class="criterio" /><input type="button" id="filtrar" class="go_filtro" value="Filtrar"/><img style="float:right;padding:8px;cursor:copy" src="../Imagenes/add.png" ';
$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/frmActualizarractas.php&x=1';
$msg .= '\'"></div></div>';
...
$msg .= '</div></th><th class="jtable-column-header" style="width:10%;"><div class="jtable-column-header-container"><span class="jtable-column-header-text">Acta</span></div></th><th class="jtable-column-header" style="width:68%;"><div class="jtable-column-header-container"><span class="jtable-column-header-text">Producto</span></div></th><th class="jtable-column-header" style="width:10%;"><div class="jtable-column-header-container"><span class="jtable-column-header-text">Cantidad</span></div></th><th class="jtable-command-column-header" style="width:1%;"></th><th class="jtable-command-column-header" style="width:1%;"></th></tr></thead><tbody>';
...
while ($row = mysqli_fetch_array($result_pag_data)) {
	...
	$msg .= '<button title="Agregar consumos" class="jtable-command-button jtable-detail-command-button"';
	$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador3.php&lista=../vistas/consumos_data.php&amp;acta=';
	$msg .= $row['acta'];
	$msg .= '\'">';
	...
	$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/frmActualizarractas.php&amp;Id=';
	$msg .= $row['codigo'];
	$msg .= '\'">';
	$msg .= ' <span>Actualizar registro</span>';
	$msg .= '</button>';
	...
	$msg .= ' onclick="location.href=\'../controlador/coactas.php?boton=eliminar&amp;Id=';
	$msg .= $row['codigo'];
	$msg .= '\'">';
