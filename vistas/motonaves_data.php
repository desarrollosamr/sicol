...
$where = "WHERE 1=1";
 if($filtropro!=""){
     $where.= " AND gc_motonaves.nombre LIKE '$filtropro%'"; 	
 }
	 
include"../clases/ConexionDatos.php";
$objConexion=Conectarse();

$query_pag_data = "SELECT * from gc_motonaves " . $where . " order by nombre LIMIT $start, $per_page";
$result_pag_data = mysqli_query($objConexion,$query_pag_data) or die('MySql Error' . mysql_error());
$msg = "";
$msg .= '<div class="jtable-title"><div class="jtable-title-text">Motonaves<input type="text" id="criterio"  class="criterio" /><input type="button" id="filtrar" class="go_filtro" value="Filtrar"/><img style="float:right;padding:8px;cursor:copy" src="../Imagenes/add.png" ';
$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador1.php&lista=../vistas/frmActualizarrmotonaves.php?x=1&modulo=recibo';
$msg .= '\'"></div></div>';
...
while ($row = mysqli_fetch_array($result_pag_data)) {
	...
	$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador1.php&lista=../vistas/frmActualizarrmotonaves.php?Id=';
	$msg .= $row['motonaveId'];
	$msg .= '&modulo=recibo';
	$msg .= '\'">';
	...
	$msg .= ' onclick="location.href=\'../controlador/comotonaves.php?boton=eliminar&amp;Id=';
	$msg .= $row['motonaveId'];
	$msg .= '\'">';
...
/* --------------------------------------------- */
$query_pag_num = "SELECT COUNT(*) AS count FROM gc_motonaves " . $where;
