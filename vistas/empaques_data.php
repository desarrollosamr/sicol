...
$query_pag_data = "SELECT * from gc_empaques " . $where . " order by nombre LIMIT $start, $per_page";
$result_pag_data = mysqli_query($objConexion,$query_pag_data) or die('MySql Error' . mysql_error());
$msg = "";
$msg .= '<div class="jtable-title"><div class="jtable-title-text">Empaques<input type="text" class="criterio" /><input type="button" id="filtrar" class="go_filtro" value="Filtrar"/><img style="float:right;padding:8px;cursor:copy" src="../Imagenes/add.png" ';
$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador1.php&lista=../vistas/frmActualizarrempaques.php?x=1&modulo=empaque';
$msg .= '\'"></div></div>';
...
while ($row = mysqli_fetch_array($result_pag_data)) {
	...
	$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador1.php&lista=../vistas/frmActualizarrempaques.php?Id=';
	$msg .= $row['codigo'];
	$msg .= '&modulo=empaque';
	$msg .= '\'">';
	...
	$msg .= ' onclick="location.href=\'../controlador/coempaques.php?boton=eliminar&amp;Id=';
	$msg .= $row['codigo'];
	$msg .= '\'">';
...
/* --------------------------------------------- */
$query_pag_num = "SELECT COUNT(*) AS count FROM gc_empaques " . $where;
