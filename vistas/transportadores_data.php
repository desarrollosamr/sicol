...
$where = "WHERE 1=1";
 if($filtropro!=""){
     $where.= " AND gc_transportadores.razon_social LIKE '$filtropro%'"; 	
 }
	 
include"../clases/ConexionDatos.php";
$objConexion=Conectarse();

$query_pag_data = "SELECT * from gc_transportadores " . $where . " order by razon_social LIMIT $start, $per_page";
$result_pag_data = mysqli_query($objConexion,$query_pag_data) or die('MySql Error' . mysql_error());
$msg = "";
$msg .= '<div class="jtable-title"><div class="jtable-title-text">Transportadores<input type="text" id="criterio" class="criterio" /><input type="button" id="filtrar" class="go_filtro" value="Filtrar"/><img style="float:right;padding:8px;cursor:copy" src="../Imagenes/add.png" ';
$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador1.php&lista=../vistas/frmActualizarrtransportadores.php?x=1&modulo=maestros';
$msg .= '\'"></div></div>';
...
$msg .= '</div></th><th class="jtable-column-header" style="width:78%;"><div class="jtable-column-header-container"><span class="jtable-column-header-text">Razon Social</span></div></th><th class="jtable-command-column-header" style="width:1%;"></th><th class="jtable-command-column-header" style="width:1%;"></th></tr></thead><tbody>';
...
while ($row = mysqli_fetch_array($result_pag_data)) {
	...
	$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador1.php&lista=../vistas/frmActualizarrtransportadores.php?Id=';
	$msg .= $row['nit'];
	$msg .= '&modulo=maestros\'">';
	...
	$msg .= ' onclick="location.href=\'../controlador/cotransportadores.php?boton=eliminar&amp;Id=';
	$msg .= $row['nit'];
	$msg .= '&modulo=maestros\'">';
...
/* --------------------------------------------- */
$query_pag_num = "SELECT COUNT(*) AS count FROM gc_transportadores " . $where;
