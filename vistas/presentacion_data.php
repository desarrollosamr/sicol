...
$where = "WHERE 1=1";
 if($filtropro!=""){
     $where.= " AND gc_presentacion.kilos LIKE '$filtropro%'"; 	
 }
	 
include"../clases/ConexionDatos.php";
$objConexion=Conectarse();

$query_pag_data = "SELECT * from gc_presentacion " . $where . " order by kilos LIMIT $start, $per_page";
$result_pag_data = mysqli_query($objConexion,$query_pag_data) or die('MySql Error' . mysql_error($objConexion));
$msg = "";
$msg .= '<div class="jtable-title"><div class="jtable-title-text">Presentaciones<input type="text" id="criterio"  class="criterio" /><input type="button" id="filtrar" class="go_filtro" value="Filtrar"/><img style="float:right;padding:8px;cursor:copy" src="../Imagenes/add.png" ';
$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador1.php&lista=../vistas/frmActualizarrpresentacion.php?x=1&modulo=maestros';
$msg .= '\'"></div></div>';
...
$msg .= '<span class="jtable-column-header-text">Codigo</span>';
$msg .= '</div></th><th class="jtable-column-header" style="width:78%;"><div class="jtable-column-header-container"><span class="jtable-column-header-text">Nombre</span></div></th><th class="jtable-command-column-header" style="width:1%;"></th><th class="jtable-command-column-header" style="width:1%;"></th></tr></thead><tbody>';
...
while ($row = mysqli_fetch_array($result_pag_data)) {
	...
	$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador1.php&lista=../vistas/frmActualizarrpresentacion.php?Id=';
	$msg .= $row['id'];
	$msg .= '&modulo=maestros';
	$msg .= '\'">';
	...
	$msg .= ' onclick="location.href=\'../controlador/copresentacion.php?boton=eliminar&amp;Id=';
	$msg .= $row['id'];
	$msg .= '\'">';
...
/* --------------------------------------------- */
$query_pag_num = "SELECT COUNT(*) AS count FROM gc_presentacion " . $where;
