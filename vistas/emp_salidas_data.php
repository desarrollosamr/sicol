...
LEFT JOIN `gc_productos` ON `gc_emp_despacho`.`producto` = `gc_productos`.`productosId` " . $where . " order by gc_emp_despacho.fecha desc LIMIT $start, $per_page";

$result_pag_data = mysqli_query($objConexion,$query_pag_data) or die('MySql Error' . mysql_error());
$msg = "";
$msg .= '<div class="jtable-title"><div class="jtable-title-text">Despacho de empaques<select class="criterio">';
	while($lista=mysqli_fetch_array($rs_empaques)) {
$msg .= '<option  value="' . $lista["codigo"] . '">' . $lista["nombre"] . '</option>';
	}
$msg .= '</select><input type="button" id="filtrar" class="go_filtro" value="Filtrar"/><input type="button"  onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador1.php&lista=../vistas/emp_salidas_data.php&modulo=empaque';
$msg .= '\'" value="Resetear"/><img style="float:right;padding:8px;cursor:copy" src="../Imagenes/add.png" ';
$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador1.php&lista=../vistas/frmActualizarrempsalidas.php?x=1&modulo=empaque';
$msg .= '\'"></div></div>';
...
$msg .= '</div></th><th class="jtable-column-header" style="width:25%;"><div class="jtable-column-header-container"><span class="jtable-column-header-text">Grado</span></div></th><th class="jtable-column-header" style="width:8%;"><div class="jtable-column-header-container"><span class="jtable-column-header-text">Cantidad</span></div></th><th class="jtable-column-header" style="width:7%;"><div class="jtable-column-header-container"><span class="jtable-column-header-text">Orden</span></div></th><th class="jtable-column-header" style="width:14%;"><div class="jtable-column-header-container"><span class="jtable-column-header-text">Operacion</span></div></th><th class="jtable-column-header" style="width:25%;"><div class="jtable-column-header-container"><span class="jtable-column-header-text">Producto</span></div></th><th class="jtable-command-column-header" style="width:1%;"></th><th class="jtable-command-column-header" style="width:1%;"></th></tr></thead><tbody>';
...
while ($row = mysqli_fetch_array($result_pag_data)) {
	...
	$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador1.php&lista=../vistas/frmActualizarrempsalidas.php?Id=';
	$msg .= $row['id'];
	$msg .= '&modulo=empaque';
	$msg .= '\'">';
	...
	$msg .= ' onclick="location.href=\'../controlador/coempsalidas.php?boton=eliminar&amp;Id=';
	$msg .= $row['id'];
	$msg .= '\'">';
...
$exportar = '<button class="go_button" style="position:relative;" onclick="location.href=\'../vistas/informes_excel.php?criterio=' . $criterio . '&modulo=emp_salidas\'" title="Exportar">Exportar</button>';
