...
$where = "WHERE 1=1";
 if($filtropro!=""){
     $where.= " AND gc_productos.nombre LIKE '$filtropro%'"; 	
 }

include"../clases/ConexionDatos.php";
$objConexion=Conectarse();

$query_pag_data = "SELECT * from gc_productos " . $where . " order by nombre LIMIT $start, $per_page";
$result_pag_data = mysqli_query($objConexion,$query_pag_data) or die('MySql Error' . mysql_error());
$msg = "";
$msg .= '<div class="jtable-title"><div class="jtable-title-text">Productos   <input type="text" id="criterio" class="criterio" /><input type="button" id="filtrar" class="go_filtro" value="Filtrar"/><img style="float:right;padding:8px;cursor:copy" src="../Imagenes/add.png" ';
$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador1.php&lista=../vistas/frmActualizarrproductos.php?x=1&modulo=empaque';
$msg .= '\'"></div></div>';
...
while ($row = mysqli_fetch_array($result_pag_data)) {
	...
	$msg .= ' onclick="location.href=\'../Plantilla/vistaPrincipal.php?pg=../vistas/paginador1.php&lista=../vistas/frmActualizarrproductos.php?Id=';
	$msg .= $row['productosId'];
	$msg .= '&modulo=empaque';
	$msg .= '\'">';
	...
	$msg .= ' onclick="location.href=\'../controlador/coproductos.php?boton=eliminar&amp;Id=';
	$msg .= $row['productosId'];
	$msg .= '\'">';
